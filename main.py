...
with open(LOG_FILE, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["timestamp", "rear_center", "rear_left", "rear_right", "collision_front", "collision_rear"])


def reader_thread_func():
    global running, data_points, last_col_front, last_col_rear
    while running:
        try:
            if ser:
                ...
                col_front = 1 if random.random() < 0.05 else 0
                col_rear = 1 if random.random() < 0.02 else 0
                raw = f"{rear_center},{rear_left},{rear_right},{col_front},{col_rear}"
                time.sleep(0.1)
            ...
            rear_center, rear_left, rear_right, col_front, col_rear = parts
            rear_center = int(rear_center); rear_left = int(rear_left); rear_right = int(rear_right)
            col_front = int(col_front); col_rear = int(col_rear)
            timestamp = datetime.now()

            with data_lock:
                data_points.append((timestamp, rear_center, rear_left, rear_right, col_front, col_rear))
                while data_points and (timestamp - data_points[0][0]).total_seconds() > window_seconds:
                    data_points.popleft()
                last_col_front = col_front
                last_col_rear = col_rear

                with open(LOG_FILE, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([timestamp.strftime("%H:%M:%S"), rear_center, rear_left, rear_right, col_front, col_rear])
...
def update_gui():
    ...
    centers = [c for _, c, _, _, _, _ in snapshot]
    lefts = [l for _, _, l, _, _, _ in snapshot]
    rights = [r for _, _, _, r, _, _ in snapshot]
    ...
    ax_plot.set_xlabel("Time (last {} s)".format(window_seconds))
    ...
    plt.setp(ax_plot.xaxis.get_majorticklabels(), rotation=30, ha="right")
    ...
    front_color = "red" if cf else "darkred"
    front_markersize = 10 if cf else 6
    ...
    # lines
    if snapshot:
        _, last_center, last_left, last_right, _, _ = snapshot[-1]
    ...
    rear_text = "REAR COLLISION!" if cr else ""
    if cr:
...
fig, (ax_plot, ax_car) = plt.subplots(1, 2, figsize=(10, 4))
